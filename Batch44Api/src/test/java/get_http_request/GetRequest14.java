 ...
 */
@Test
public void test14Matchers(){
	spec03.pathParams("first","tp-customers","second","110472");
	Response response =given().spec(spec03).header("Authorization","Bearer " +generateToken()).when().get("/{first}/{second}");
	...
	response.then().assertThat().body("firstName", Matchers.equalTo("Melva")
			,"lastName",Matchers.equalTo("Bernhard")
					...
					,"country.name", Matchers.equalTo("San Jose")
					,"user.login",Matchers.equalTo("delilah.metz"));
}
	@Test
	public void test14JsonPath(){
		spec03.pathParams("first","tp-customers","second","110472");
		Response response =given().spec(spec03).header("Authorization","Bearer " +generateToken()).when().get("/{first}/{second}");
